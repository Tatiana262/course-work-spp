...
func (a *PropertyFileStorageAdapter) Save(_ context.Context, record domain.RealEstateRecord) error {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	if err != nil {
		return fmt.Errorf("failed to format record to pretty JSON for URL %s: %w", record.General.Source, err)
	}
