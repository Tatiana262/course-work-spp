...
func (uc *ProcessTaskResultUseCase) Execute(ctx context.Context, taskID uuid.UUID, results map[string]int) error {
	...
	// Шаг 1: Атомарно инкрементируем счетчики в БД
	...
	// Шаг 3: Проверяем, не пора ли завершать задачу
	summary := updatedTask.ResultSummary
	...
	// Проверяем, было ли в этом сообщении установлено `expected_results_count`
	// Если да, и `expectedCount` теперь > 0, значит, пришла "команда на завершение".
	...
	if updatedTask.Type == "FIND_NEW" {
		...
		// 1. Мы знаем, сколько ждать поисков (`expectedCount > 0`)
		// 2. Все поиски завершились
		// 3. Все найденные ссылки обработаны
	...
	} else { // Для 'ACTUALIZE_ACTIVE', 'ACTUALIZE_ARCHIVED' и т.д.
		totalProcessed := getIntFromSummary(summary, "total_processed")
		
		// Условие завершения для детерминированных задач:
		// 1. Мы знаем, сколько ждать результатов (`expectedCount > 0`)
		// 2. Все результаты получены
		if expectedCount > 0 && totalProcessed >= expectedCount {
			shouldComplete = true
	...
	// Шаг 4: Если нужно, завершаем задачу
	if shouldComplete {
		uc.markTaskAsCompleted(ctx, updatedTask, logMessage)
	}

	ucLogger.Debug("Notifying clients about task progress", nil)
	// Шаг 5: Отправляем уведомление о текущем состоянии
	uc.notifier.Notify(ctx, port.TaskEvent{Type: "task_updated", Data: *updatedTask})
	ucLogger.Info("Use case finished successfully", port.Fields{
		"task_status": updatedTask.Status,
	})
