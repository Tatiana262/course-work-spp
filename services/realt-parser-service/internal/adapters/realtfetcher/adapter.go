...
func NewRealtFetcherAdapter(baseURL string) (*RealtFetcherAdapter, error) {

	// Создаем родительский коллектор
	...
	if err != nil {
		return nil, fmt.Errorf("RealtFetcherAdapter: Failed to set limit rule: %w", err)
	}
